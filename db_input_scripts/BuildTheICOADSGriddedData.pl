...
print PSQL ("SET SEARCH_PATH TO physical; \n");

print PSQL ("DROP TABLE physical.icoads_gridded_sst CASCADE; \n");
#
##print PSQL ("CREATE TABLESPACE dbspace LOCATION \'G://pGdata\'; \n");
#
print PSQL ("SET default_tablespace = dbspace; \n");
#
print PSQL ("CREATE TABLE physical.icoads_gridded_sst (
year int,
month int,
bsz int,
blo float,
...
lon float,
lat float,
type varchar(3)) WITH OIDS;\n");


print PSQL ("COMMENT ON TABLE  physical.icoads_gridded_sst IS 'These are global gridded data from ICOADs';\n");
print PSQL ("COMMENT ON COLUMN physical.icoads.gridded_sst.year IS 'Year';\n");
print PSQL ("COMMENT ON COLUMN physical.icoads_gridded_sst.month IS 'Month';\n");
print PSQL ("COMMENT ON COLUMN physical.icoads_gridded_sst.bsz IS 'Box size: 1 = 1degree latitude x longitude 2 = 2degree latitude x longitude';\n");
print PSQL ("COMMENT ON COLUMN physical.icoads_gridded_sst.blo IS 'box left (W) corner longitude (E)';\n");
print PSQL ("COMMENT ON COLUMN physical.icoads_gridded_sst.bla IS 'box lower (S) corner latitude (+N, -S)';\n");
print PSQL ("COMMENT ON COLUMN physical.icoads_gridded_sst.pid2 IS 'PID2: 0 = standard statistics (3.5 sigma trimming limits; ship data) 1 = enhanced statistics (4.5 sigma trimming limits; ship + other data)';\n");
print PSQL ("COMMENT ON COLUMN physical.icoads_gridded_sst.M IS 'mean';\n");
print PSQL ("COMMENT ON COLUMN physical.icoads_gridded_sst.N IS 'number of observations';\n");
print PSQL ("COMMENT ON COLUMN physical.icoads_gridded_sst.S IS 'standard deviation';\n");
print PSQL ("COMMENT ON COLUMN physical.icoads_gridded_sst.D IS 'mean day of month of the observations';\n");
print PSQL ("COMMENT ON COLUMN physical.icoads_gridded_sst.HT IS 'fraction of observations in daylight';\n");
print PSQL ("COMMENT ON COLUMN physical.icoads_gridded_sst.lon IS 'mean longitude of observations';\n");
print PSQL ("COMMENT ON COLUMN physical.icoads_gridded_sst.lat IS 'mean latitude of observations';\n");
print PSQL ("COMMENT ON COLUMN physical.icoads_gridded_sst.type IS 'type of data, e.g. SST=sea surface temperature';\n");


## FILL THE TABLE

open(DATAFILE, "ls g:/bearedo/Database/ICOADS-Global-Gridded/sst/tmp/*dat |");
while(<DATAFILE>){
s/^\s*(.*)\s*$/$1/;
chomp;
open(FILE,"<$_") or die "Cannot open file $_: $!\n";
print PSQL ("\\COPY physical.icoads_gridded_sst FROM \'$_\' WITH delimiter \'|\' null as \'-9999.00\'\n");
print "IMPORTING: $_\n";}
close(DATAFILE);

print PSQL ("ALTER TABLE physical.icoads_gridded_sst ADD PRIMARY KEY (year,month,blo,bla);
\n");
#
## Create indices ###

print PSQL ("CREATE INDEX icoads_gridded_sst_month ON physical.icoads_gridded_sst (month);\n");
print PSQL ("CREATE INDEX icoads_gridded_sst_year ON physical.icoads_gridded_sst (year);\n");
print PSQL ("CREATE INDEX icoads_gridded_sst_lat ON physical.icoads_gridded_sst (lat);\n");
print PSQL ("CREATE INDEX icoads_gridded_sst_lon ON physical.icoads_gridded_sst (lon);\n");

### Add on geometry point ###

print PSQL ("SET SEARCH_PATH to public;\n");
print PSQL ("SELECT addgeometrycolumn('','physical','icoads_gridded_sst','the_point',4326,'POINT',2);\n");
print PSQL ("UPDATE physical.icoads_gridded_sst SET the_point = GeomFromEWKT('SRID=4326;POINT(' || icoads_gridded_sst.lon || ' ' || icoads_gridded_sst.lat || ')');\n");
print PSQL ("CREATE INDEX icoads_gridded_sst_the_point ON physical.icoads_gridded_sst USING GIST (the_point);\n");
